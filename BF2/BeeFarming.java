...
//    Music music = new Music("flourish.mid");
	...
	public BeeFarming(String outPutFileName) throws InterruptedException{
        ...
        setTitle("BeeFarming Game"); //调用父类构造函数               
	 	imgBee = getToolkit().getImage("change.png");
//		imgBee2 = getToolkit().getImage("change.png");
//		imgBG = this.getToolkit().getImage("green.jpg");
	    Image[] imgFlw = new Image[3];
//		imgFlw[0] = this.getToolkit().getImage("change.png");
//		imgFlw[1] = this.getToolkit().getImage("change.png");
//		imgFlw[2] = this.getToolkit().getImage("change.png");
	    for (int i = 0; i < imgFlw.length; i++) {  
	        imgFlw[i] = imgBee;  
	    }
...
//		container.setLayout(null);//不能用BorderLayout，否杂JPanel会被改变大小
		...
		bees[9] = new Hornet(9,690,500,240,true,imgBee);
/* 		pGround.add(bees[9]);
		pGround.add(bees[1]);
		pGround.add(bees[2]);
		pGround.add(bees[3]); */
        ...
        try{
			...
			while(input.hasNext()){
				flws[i] = new Flower(Integer.parseInt(input.next()),30+Integer.parseInt(input.next()),
									...
									imgFlw[Integer.parseInt(input.next())],null);
				i++;
			}
...
//			System.out.println(e);
        ...
        //setVisible(false); //设置窗口可视
        //setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);     //关闭窗口时退出程序    
		

		
		Thread thread = new Thread(new GodThread());
		thread.start();
		...
		if(time==4000||count==0||countflower==0){
			...
			try{
				...
				PrintWriter out = new PrintWriter(
					...
					out.println("" + goals + "," + BeeFarming.getHoney() + "," + count + "," + atime);
					out.close();
					}catch(FileNotFoundException e){
...
//						System.out.println("Error:Cannot write to file.");
		...
		if(angle_damp==0){
			for(int i=1;i<10;i++)
				if(bees[i]!=null&&bees[i].getlive()==true)bees[i].search();//蜜蜂是否存在，死了会从数组里删除
//					if(bees[i].getlive()==true)bees[i].search();					
		}
		//延既定飞行路线飞行直到下一次搜索
		for(int i=1;i<10;i++)
			if(bees[i]!=null&&bees[i].getlive()==true)bees[i].flying(angle_damp);//蜜蜂是否存在，死了会从数组里删除
